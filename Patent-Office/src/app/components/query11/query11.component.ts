...
export class Query11Component implements OnInit, AfterViewInit {
  ...
  constructor(private _queryService: QueryService) {
    ...
    this.displayedColumns = ['name', 'area', 'borders'];
    this.data = [];
  }
  ...
  private async getData(): Promise<void> {
    try {
      const data = await this._queryService.getQuery('report-11');
      if (data['code'] === 200) {
        this.data = data['data'];
        this.dataSource.data = this.data;
      }
